...
d <- rnormmix(10000, prop=prop, mu=mu, sigma=sigma) %>%
  as.data.frame()
plt <- ggplot(data.frame("y"=y), aes(x=y)) + geom_histogram(aes(y=..density..), bins=45, fill="grey", colour="black") +
  geom_density(aes(x=y), d, size = 0.7, fill = "blue", alpha = 0.1) + plotTheme() +
  labs(title="Otsu Initializations", x="Intensity") +
  theme(plot.margin = unit(c(0.4, 0, -2.4, 0.1), "cm"))
...
```{r}
tc.hmrfem <- mritc.hmrfem(y, mrispatial$neighbors, mrispatial$blocks, mu=mu, sigma=sigma, verbose = TRUE)
```
```{r}
tc.em <- mritc.em(y, prop=prop, mu=mu, sigma=sigma, verbose = TRUE)
```
```{r}
tc.mcmcsub.otsu <- mritc.bayes(y, mrispatial$neighbors,
 mrispatial$blocks, mrispatial$sub, mrispatial$subvox, mu = mu,
...
measureMRI(T1[mask == 1], truth, tc.hmrfem$prob)
```
